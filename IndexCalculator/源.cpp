	...
	{ 
		...
		for (k = 1; k <= j; k++)  //对第一个数的分母枚举
		{
			...
			for (l = 1; l <= k; l++)  //对第二个数的分母枚举
			{
				...
				//其中一个数将会不小于I/(I+1)
				//第一个数
				for (b[0] = i*c[0]/(i+1); b[0] < c[0]; b[0]++)  //对第一个数的分子枚举
				{
					result = true;
					for (b[1] = (c[0] - b[0]) * c[1] / c[0]; b[1] < c[1]; b[1]++)  //对第二个数的分子枚举
					{
						result = false;	
...
//						}
				...
				{
					result = true;
					for (b[1] = i * c[1] / (i + 1); b[1] < c[1]; b[1]++)  //对第二个数的分子枚举
					{
						result = false;
						b[2] = 2 * c[0] * c[1] - b[0] * c[1] - b[1] * c[0];
						c[2] = c[0] * c[1];
...
//						}
				...
				{
					result = true;
					for (b[1] = (c[0]-b[0])*c[1]/c[0]; b[1] <= ((i+1)*(c[0]-b[0])+c[0])*c[1]/(c[0]*(i+1)) && b[1] < c[1]; b[1]++)  //对第二个数的分子枚举
					{
						result = false;
						b[2] = 2 * c[0] * c[1] - b[0] * c[1] - b[1] * c[0];
						c[2] = c[0] * c[1];
